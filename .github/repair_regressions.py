...
if marker in s and "_systemVolumeChannel" not in s:
    s=s.replace(marker,"  static const _resumeSongIdKey = 'playback_resume_song_id';\n  static const MethodChannel _systemVolumeChannel = MethodChannel('com.example.resonate/media_store');\n",1)
needle="      _resumeSongId = prefs.getString(_resumeSongIdKey);\n"
if needle in s and "await _syncSystemVolume();" not in s:
    s=s.replace(needle, needle+"      await _syncSystemVolume();\n",1)
...
if 'import android.media.AudioManager' not in s:
    s=s.replace('import android.media.audiofx.BassBoost\n', 'import android.media.AudioManager\nimport android.media.audiofx.BassBoost\n', 1)
...
insert_before='''    private fun pickFolder(result: MethodChannel.Result) {'''
methods='''    private fun getSystemVolume(result: MethodChannel.Result) {\n        try {\n            val audioManager = getSystemService(AUDIO_SERVICE) as AudioManager\n            result.success(mapOf("current" to audioManager.getStreamVolume(AudioManager.STREAM_MUSIC), "max" to audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC)))\n        } catch (e: Exception) { result.error("SYSTEM_VOLUME_ERROR", e.message, null) }\n    }\n\n    private fun setSystemVolume(value: Double, result: MethodChannel.Result) {\n        try {\n            val audioManager = getSystemService(AUDIO_SERVICE) as AudioManager\n            val max = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC)\n            val target = (value.coerceIn(0.0, 1.0) * max).roundToInt()\n            audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, target, 0)\n            result.success(true)\n        } catch (e: Exception) { result.error("SYSTEM_VOLUME_ERROR", e.message, null) }\n    }\n\n'''
if insert_before in s and 'private fun getSystemVolume' not in s: s=s.replace(insert_before,methods+insert_before,1)
...
s=p.read_text()
s=s.replace("duration: const Duration(milliseconds: 1800)", "duration: const Duration(milliseconds: 3200)")
s=s.replace("const Duration(milliseconds: 500)", "const Duration(milliseconds: 900)", 1)
s=s.replace('class _WaveSeekBar extends StatelessWidget {','class _WaveSeekBar extends StatefulWidget {',1)
start=s.find('class _WaveSeekBar extends StatefulWidget {')
end=s.find('\nclass _WaveSeekPainter',start)
if start<0 or end<0: raise SystemExit('wave bounds not found')
widget='''class _WaveSeekBar extends StatefulWidget {\n  final double value; final double max; final ValueChanged<double> onStart; final ValueChanged<double> onUpdate; final ValueChanged<double> onEnd;\n  const _WaveSeekBar({required this.value, required this.max, required this.onStart, required this.onUpdate, required this.onEnd});\n  @override State<_WaveSeekBar> createState() => _WaveSeekBarState();\n}\n\nclass _WaveSeekBarState extends State<_WaveSeekBar> {\n  double? _interactionValue;\n  double _valueFor(Offset local, double width) => width <= 0 ? 0 : (local.dx / width).clamp(0.0, 1.0) * widget.max;\n  @override Widget build(BuildContext context) {\n    return LayoutBuilder(builder: (context, constraints) {\n      final display = _interactionValue ?? widget.value;\n      return GestureDetector(\n        behavior: HitTestBehavior.opaque,\n        onHorizontalDragStart: (d) { final v=_valueFor(d.localPosition,constraints.maxWidth); setState(()=>_interactionValue=v); widget.onStart(v); },\n        onHorizontalDragUpdate: (d) { final v=_valueFor(d.localPosition,constraints.maxWidth); setState(()=>_interactionValue=v); widget.onUpdate(v); },\n        onHorizontalDragEnd: (_) { final v=_interactionValue ?? widget.value; setState(()=>_interactionValue=null); widget.onEnd(v); },\n        onTapDown: (d) { final v=_valueFor(d.localPosition,constraints.maxWidth); setState(()=>_interactionValue=v); widget.onStart(v); },\n        onTapUp: (d) { final v=_valueFor(d.localPosition,constraints.maxWidth); setState(()=>_interactionValue=null); widget.onEnd(v); },\n        child: SizedBox(height:64, child: CustomPaint(painter:_WaveSeekPainter(progress:widget.max<=0?0:display/widget.max,color:Theme.of(context).colorScheme.primary,muted:Theme.of(context).colorScheme.outlineVariant))),\n      );\n    });\n  }\n}\n'''
s=s[:start]+widget+s[end:]
old='''              Consumer<IntelligenceProvider>(\n                builder: (context, intelligence, _) {'''
if old in s and 'const AutopilotTakeoverCard(),\n              const SizedBox(height: 10),\n              Consumer<IntelligenceProvider>' not in s:
    s=s.replace(old,'              const AutopilotTakeoverCard(),\n              const SizedBox(height: 10),\n'+old,1)

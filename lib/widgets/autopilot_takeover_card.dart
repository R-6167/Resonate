...
class _AutopilotTakeoverCardState extends State<AutopilotTakeoverCard> {
  ...
  @override void initState() { super.initState(); _profileFuture = _loadProfile(); }
    ...
    return FutureBuilder<IntelligenceCompanionProfile>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: [
          if (pendingSong != null)
            Card(color: scheme.primaryContainer, child: Padding(padding: const EdgeInsets.fromLTRB(14, 12, 8, 10), child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 2), Text('Next: ${pendingSong!.title}', maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleSmall),
          ...
          Card(child: Padding(padding: const EdgeInsets.fromLTRB(14, 13, 14, 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
            ...
            if (profile == null) ...[const SizedBox(height: 8), const LinearProgressIndicator(minHeight: 2)] else ...[
              const SizedBox(height: 5), Text(profile.tendency, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
              const SizedBox(height: 3), Text(profile.explanation, style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 9), Wrap(spacing: 6, runSpacing: 6, children: [
              ...
              const SizedBox(height: 10), Text('How I see your listening', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: scheme.primary)),
              const SizedBox(height: 7), _SignalBar(label: 'Familiarity', value: profile.familiarityAffinity), _SignalBar(label: 'Exploration', value: profile.explorationAffinity),
              _SignalBar(label: 'Artist variety', value: profile.artistDiversity), _SignalBar(label: 'Feedback alignment', value: profile.feedbackAlignment),
              const SizedBox(height: 8), Text(profile.context, style: Theme.of(context).textTheme.labelMedium),
            ],
          ]))),
        ]);
      },
...
class _SignalBar extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
      ...
      Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: value.clamp(0.0, 1.0).toDouble(), minHeight: 5, backgroundColor: scheme.surfaceContainerHighest))),
      const SizedBox(width: 8), SizedBox(width: 34, child: Text('${(value * 100).round()}%', textAlign: TextAlign.end, style: Theme.of(context).textTheme.labelSmall)),
    ]));
  }
}

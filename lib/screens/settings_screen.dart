...
class SettingsScreen extends StatelessWidget {
    ...
    body: ListView(
      ...
      children: [
        _category(context, 'Playback', 'How Resonate behaves while you listen', Icons.play_circle_outline, [
          ...
          _entry('Queue behaviour', 'Queue, anticipation and next-track handling', Icons.queue_music_rounded, () => _info(context, 'Queue behaviour', 'Your current queue remains authoritative. Intelligence can prepare additions, but never becomes the owner of playback.')),
        ...
        _category(context, 'Privacy', 'Keep your listening data yours', Icons.lock_outline_rounded, [
          ...
          _entry('Listening history', 'Playback events used for learning', Icons.history_rounded, () => _info(context, 'Listening history', 'Listening events include completion, skips and transitions.')),
        ...
        _category(context, 'About', 'Resonate • Aetherion', Icons.info_outline_rounded, [
          _entry('About & How to use', 'Guide, privacy notes and copyright', Icons.menu_book_outlined, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AboutScreen()))),
        ]),
      ],
    ),
  );
  ...
  Widget _category(BuildContext context, String title, String subtitle, IconData icon, List<_SettingEntry> entries) => Card(
    margin: const EdgeInsets.only(bottom: 10),
    child: ExpansionTile(
      leading: Icon(icon, color: Theme.of(context).colorScheme.primary),
      title: Text(title, style: Theme.of(context).textTheme.titleMedium),
      subtitle: Text(subtitle),
      children: [for (final entry in entries) ListTile(contentPadding: const EdgeInsets.only(left: 24, right: 16), leading: Icon(entry.icon), title: Text(entry.title), subtitle: Text(entry.subtitle), trailing: const Icon(Icons.chevron_right), onTap: entry.onTap)],
    ),
  );

  Future<void> _showTheme(BuildContext context) async {
    await showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheet) => Consumer<ThemeProvider>(builder: (_, theme, __) => SafeArea(child: ListView(shrinkWrap: true, children: [RadioListTile<String>(title: const Text('System'), value: 'system', groupValue: theme.useSystemTheme ? 'system' : (theme.isDarkMode ? 'dark' : 'light'), onChanged: (_) => theme.toggleSystemTheme()), RadioListTile<String>(title: const Text('Light'), value: 'light', groupValue: theme.useSystemTheme ? 'system' : (theme.isDarkMode ? 'dark' : 'light'), onChanged: (_) { if (theme.useSystemTheme) theme.toggleSystemTheme(); if (theme.isDarkMode) theme.toggleTheme(); }), RadioListTile<String>(title: const Text('Dark'), value: 'dark', groupValue: theme.useSystemTheme ? 'system' : (theme.isDarkMode ? 'dark' : 'light'), onChanged: (_) { if (theme.useSystemTheme) theme.toggleSystemTheme(); if (!theme.isDarkMode) theme.toggleTheme(); })]))));
  }

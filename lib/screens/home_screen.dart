...
class _HomeDashboardState extends State<_HomeDashboard> with SingleTickerProviderStateMixin {
  ...
  Widget _sectionTitle(BuildContext context, String text) => Padding(
    ...
    child: Text(text, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w750)),
  );
}
...
class _ListeningPulse extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(width: 56, height: 34, child: AnimatedBuilder(animation: animation, builder: (_, __) {
      return Row(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.center, children: List.generate(3, (i) {
        final t = (animation.value + i * .18) % 1.0;
        final opacity = .35 + (.65 * (0.5 + 0.5 * (1 - (2 * t - 1).abs())));
        final y = 2 + 7 * (0.5 + 0.5 * (1 - (2 * t - 1).abs()));
        return Padding(padding: const EdgeInsets.only(left: 5), child: Opacity(opacity: opacity, child: Transform.translate(offset: Offset(0, y - 5), child: Container(width: 5, height: 5, decoration: BoxDecoration(color: scheme.primary, shape: BoxShape.circle)))));
      });
    }));
  }
}
...
class _ResonateWordmark extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return ShaderMask(shaderCallback: (bounds) => LinearGradient(colors: [scheme.primary, scheme.secondary]).createShader(bounds), child: const Text('Resonate', style: TextStyle(fontFamily: 'serif', fontSize: 27, fontWeight: FontWeight.w900, fontStyle: FontStyle.italic, letterSpacing: -1.4, color: Colors.white)));
  }
}
...
class _IntelligenceHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final description = autopilot
        ? 'Autopilot is quietly choosing what fits the session. You stay in control.'
...
class _AnticipationCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Card(color: scheme.primaryContainer.withOpacity(.72), child: Padding(padding: const EdgeInsets.fromLTRB(16, 16, 10, 10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(autopilot ? Icons.smart_toy_rounded : Icons.auto_awesome, color: scheme.primary), const SizedBox(width: 8), Expanded(child: Text(autopilot ? 'Next-track decision' : 'Picked for this moment', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: scheme.primary))), Text('${(item.confidence * 100).round()}%', style: Theme.of(context).textTheme.labelLarge)]),
      ...
      Align(alignment: Alignment.centerRight, child: FilledButton.icon(onPressed: () async { final i = songs.indexWhere((s) => s.id == item.song.id); await music.playSong(item.song, queue: songs.cast(), startIndex: i < 0 ? 0 : i); }, icon: const Icon(Icons.play_arrow_rounded), label: Text(autopilot ? 'Play now' : 'Play this'))),
    ])));
  }
}
...
class _RecommendationTile extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 9), child: ListTile(
      ...
      trailing: Row(mainAxisSize: MainAxisSize.min, children: [_FeedbackButtons(songId: item.song.id), IconButton(tooltip: 'Play', icon: const Icon(Icons.play_arrow_rounded), onPressed: () async { final i = songs.indexWhere((s) => s.id == item.song.id); await music.playSong(item.song, queue: songs.cast(), startIndex: i < 0 ? 0 : i); })]),
    ));
  }
}

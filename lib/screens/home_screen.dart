...
class _HomeDashboard extends StatelessWidget {
  ...
  List<Widget> _recommendations(IntelligenceProvider intelligence, List<dynamic> songs) {
    ...
    for (final item in recommendations) {
      items.add(_RecommendationTile(item: item, songs: songs));
    }
    return items;
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    if (intelligence.isEnabled) {
      ...
      children.add(_SessionCard(intelligence: intelligence));
      children.add(const SizedBox(height: 14));
      children.add(const EvolvingMixCard());
    }

    if (music.currentSong != null) {
      children.add(const SizedBox(height: 20));
      children.add(_sectionTitle(context, 'Now playing'));
    ...
    if (intelligence.isEnabled && next != null) {
      children.add(const SizedBox(height: 20));
      children.add(_sectionTitle(context, intelligence.isAutopilot ? 'Next up, intelligently' : 'A little nudge'));
      children.add(_AnticipationCard(item: next, songs: recommendationQueue.isEmpty ? songs : recommendationQueue));
    }

    children.add(const SizedBox(height: 20));
    children.add(_sectionTitle(context, intelligence.isEnabled ? 'More music for this moment' : 'Suggested from your library'));
    children.addAll(_recommendations(intelligence, recommendationQueue.isEmpty ? songs : recommendationQueue));
    children.add(const SizedBox(height: 30));

    return Scaffold(
      appBar: AppBar(title: const Text('Resonate')),
      body: RefreshIndicator(onRefresh: intelligence.refreshRecommendations, child: ListView(padding: const EdgeInsets.fromLTRB(18, 0, 18, 34), children: children)),
    );
  }
...
class _IntelligenceHero extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: LinearGradient(colors: [scheme.primaryContainer, scheme.surfaceContainerHighest]), border: Border.all(color: scheme.outlineVariant)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(intelligence.isAutopilot ? Icons.smart_toy_rounded : Icons.auto_awesome), const SizedBox(width: 10), Expanded(child: Text('Resonate Intelligence', style: Theme.of(context).textTheme.titleLarge)), Text(intelligence.isEnabled ? intelligence.autonomyLabel : 'OFF')]),

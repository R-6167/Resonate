...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _openSongEq(BuildContext context, String songId) async {
    ...
    final action = await PlayerActionOverlay.show<String>(context: context, icon: Icons.equalizer_rounded, title: 'Song EQ', child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.save_outlined), title: const Text('Save EQ for this song'), onTap: () => Navigator.pop(context, 'save')), ListTile(leading: const Icon(Icons.download_outlined), title: const Text('Load saved EQ'), onTap: () => Navigator.pop(context, 'load'))]));
    if (action == 'save') await eq.saveSongProfile(songId);
    if (action == 'load') { final found = await eq.loadSongProfile(songId); if (context.mounted && !found) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No saved EQ profile for this song.'))); }
  }

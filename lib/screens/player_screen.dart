...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Consumer<MusicProvider>(builder: (context, music, _) {
      ...
      final repeatLabel = switch (music.repeatMode) {
      ...
      return ListView(padding: const EdgeInsets.fromLTRB(18, 8, 18, 28), children: [
        ...
        Card(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          ...
          IconButton(tooltip: repeatLabel, icon: Icon(repeatIcon, color: music.repeatMode != PlaybackRepeatMode.off ? scheme.primary : null), onPressed: () {
            final next = switch (music.repeatMode) { PlaybackRepeatMode.off => PlaybackRepeatMode.all, PlaybackRepeatMode.all => PlaybackRepeatMode.one, PlaybackRepeatMode.one => PlaybackRepeatMode.off };
  ...
  Future<void> _showMoreOptions(BuildContext context, MusicProvider music) async { final playback = context.read<PlaybackFeaturesProvider>(); await PlayerActionOverlay.show<void>(context: context, icon: Icons.more_horiz_rounded, title: 'Playback options', child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.speed_rounded), title: const Text('Speed'), trailing: Text('${playback.speed.toStringAsFixed(2)}×'), onTap: () => _showSpeed(context, playback)), ListTile(leading: const Icon(Icons.tune_rounded), title: const Text('Pitch'), trailing: Text('${playback.pitch.toStringAsFixed(2)}×'), onTap: () => _showPitch(context, playback)), ListTile(leading: Icon(Icons.compare_arrows_rounded, color: music.crossfadeEnabled ? Theme.of(context).colorScheme.primary : null), title: const Text('Crossfade'), subtitle: Text(music.crossfadeEnabled ? '${music.crossfadeDurationMs ~/ 1000}s • ${music.crossfadeFadeType.replaceAll('_', ' ')}' : 'Off'), onTap: () async { await music.setCrossfadeEnabled(!music.crossfadeEnabled); Navigator.pop(context); }), SwitchListTile(secondary: const Icon(Icons.volume_down_rounded), title: const Text('Volume normalization'), subtitle: Text('Target ${playback.targetLoudness.toStringAsFixed(0)} LUFS'), value: playback.normalizationEnabled, onChanged: playback.setNormalizationEnabled)])); }

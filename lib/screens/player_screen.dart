...
class PlayerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Now Playing'), actions: [
        Consumer<MusicProvider>(builder: (_, music, __) => IconButton(tooltip: 'Stop', icon: const Icon(Icons.stop_rounded), onPressed: music.currentSong == null ? null : music.stop)),
      ]),
      body: Consumer<MusicProvider>(builder: (context, music, _) {
        ...
        final value = music.currentPosition.inMilliseconds.toDouble().clamp(0, max);

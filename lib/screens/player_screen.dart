...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _showVolume(BuildContext context) async {
    ...
    try {
      await PlayerActionOverlay.show<void>(
        context: context, icon: Icons.volume_up_rounded, title: 'Volume',
        child: ValueListenableBuilder<double>(valueListenable: _systemVolume, builder: (_, value, __) => Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    body: Consumer<MusicProvider>(builder: (context, music, _) {
      ...
      return ListView(padding: const EdgeInsets.fromLTRB(18, 8, 18, 28), children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Consumer<PlaybackFeaturesProvider>(builder: (_, playback, __) => Row(mainAxisSize: MainAxisSize.min, children: [
            IconButton(tooltip: playback.sleepTimerActive ? 'Sleep timer: ${playback.sleepTimerLabel}' : 'Sleep timer', icon: Icon(Icons.timer_outlined, color: playback.sleepTimerActive ? scheme.primary : null), onPressed: () => _showSleepTimer(context, playback)),
            if (playback.sleepTimerActive) Text(playback.sleepTimerLabel, style: Theme.of(context).textTheme.labelMedium),
          ])),
        ...
        Consumer<IntelligenceProvider>(builder: (context, intelligence, _) {
          ...
          return _IntelligenceNextCard(item: item);
        }),
      ]);
    }),
  );
  ...
  Future<void> _showMoreOptions(BuildContext context) async {
    ...
    await PlayerActionOverlay.show<void>(context: context, icon: Icons.more_horiz_rounded, title: 'Playback options', child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  Future<void> _showSpeed(BuildContext context, PlaybackFeaturesProvider playback) async {
    await PlayerActionOverlay.show<void>(context: context, icon: Icons.speed_rounded, title: 'Playback speed', child: StatefulBuilder(builder: (_, setState) => Column(mainAxisSize: MainAxisSize.min, children: [
      Text('${playback.speed.toStringAsFixed(2)}×', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
  ...
  Future<void> _showPitch(BuildContext context, PlaybackFeaturesProvider playback) async {
    await PlayerActionOverlay.show<void>(context: context, icon: Icons.tune_rounded, title: 'Pitch', child: StatefulBuilder(builder: (_, setState) => Column(mainAxisSize: MainAxisSize.min, children: [
      Text('${playback.pitch.toStringAsFixed(2)}×', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
  ...
  Future<void> _openSongEq(BuildContext context, String songId) async {
    ...
    final action = await PlayerActionOverlay.show<String>(context: context, icon: Icons.equalizer_rounded, title: 'Song EQ', child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.save_outlined), title: const Text('Save EQ for this song'), onTap: () => Navigator.pop(context, 'save')),
      ListTile(leading: const Icon(Icons.download_outlined), title: const Text('Load saved EQ'), onTap: () => Navigator.pop(context, 'load')),
    ]));
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('I think you want this next', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: scheme.primary)),

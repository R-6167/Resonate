...
class _PlayerScreenState extends State<PlayerScreen> with SingleTickerProviderStateMixin {
  ...
  late final AnimationController _titleMotion = AnimationController(vsync: this, duration: const Duration(milliseconds: 8200))..repeat(reverse: true);

  @override
  void initState() { super.initState(); _loadSystemVolume(); }

  Future<void> _loadSystemVolume() async {
    try { final volume = await FlutterVolumeController.getVolume(); if (volume != null) _systemVolume.value = volume.clamp(0.0, 1.0).toDouble(); } catch (_) {}
  }

  @override
  void dispose() { _titleMotion.dispose(); _systemVolume.dispose(); super.dispose(); }
  ...
  Future<void> _showVolume(BuildContext context) async {
    ...
    try {
      await PlayerActionOverlay.show<void>(context: context, icon: Icons.volume_up_rounded, title: 'Volume', child: ValueListenableBuilder<double>(valueListenable: _systemVolume, builder: (_, value, __) => Column(mainAxisSize: MainAxisSize.min, children: [
        const SizedBox(height: 8),
        Row(children: [Icon(value <= 0 ? Icons.volume_off_rounded : Icons.volume_up_rounded, size: 28), Expanded(child: Slider(min: 0, max: 1, value: value, onChanged: (next) async { _systemVolume.value = next; await FlutterVolumeController.setVolume(next); })), SizedBox(width: 52, child: Text('${(value * 100).round()}%', textAlign: TextAlign.end))]),
        const SizedBox(height: 8),
      ])));
    } finally { FlutterVolumeController.removeListener(); }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Consumer<MusicProvider>(builder: (context, music, _) {
      ...
      final repeatLabel = switch (music.repeatMode) { PlaybackRepeatMode.all => 'Repeat all', PlaybackRepeatMode.one => 'Repeat one', PlaybackRepeatMode.off => 'Repeat off' };
      final repeatIcon = music.repeatMode == PlaybackRepeatMode.one ? Icons.repeat_one_rounded : Icons.repeat_rounded;
      return ListView(padding: const EdgeInsets.fromLTRB(18, 8, 18, 28), children: [
        ...
        _ScrollingSongTitle(title: song.title, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold), animation: _titleMotion),
        const SizedBox(height: 5),
        Text(song.artist, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyLarge),
        Text(song.album, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium),
        ...
        Card(child: SingleChildScrollView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2), child: Row(mainAxisSize: MainAxisSize.min, children: [
          IconButton(tooltip: music.shuffleEnabled ? 'Shuffle on' : 'Shuffle off', icon: Icon(Icons.shuffle_rounded, color: music.shuffleEnabled ? scheme.primary : null), onPressed: () => music.setShuffleEnabled(!music.shuffleEnabled)),
          IconButton(tooltip: repeatLabel, icon: Icon(repeatIcon, color: music.repeatMode != PlaybackRepeatMode.off ? scheme.primary : null), onPressed: () { final next = switch (music.repeatMode) { PlaybackRepeatMode.off => PlaybackRepeatMode.all, PlaybackRepeatMode.all => PlaybackRepeatMode.one, PlaybackRepeatMode.one => PlaybackRepeatMode.off }; music.setRepeatMode(next); }),
          IconButton(tooltip: music.crossfadeEnabled ? 'Crossfade on' : 'Crossfade off', icon: Icon(Icons.compare_arrows_rounded, color: music.crossfadeEnabled ? scheme.primary : null), onPressed: () => music.setCrossfadeEnabled(!music.crossfadeEnabled)),
          ...
          ValueListenableBuilder<double>(valueListenable: _systemVolume, builder: (_, volume, __) => IconButton(tooltip: 'Volume', icon: Icon(volume <= 0 ? Icons.volume_off_rounded : Icons.volume_up_rounded), onPressed: () => _showVolume(context))),
          Consumer<PlaybackFeaturesProvider>(builder: (_, playback, __) => IconButton(tooltip: playback.sleepTimerActive ? 'Sleep timer: ${playback.sleepTimerLabel}' : 'Sleep timer', icon: Icon(Icons.timer_outlined, color: playback.sleepTimerActive ? scheme.primary : null), onPressed: () => _showSleepTimer(context, playback))),
          IconButton(tooltip: 'Per-song EQ', icon: const Icon(Icons.equalizer_rounded), onPressed: () => _openSongEq(context, song.id)),
          IconButton(tooltip: 'More playback options', icon: const Icon(Icons.more_vert_rounded), onPressed: () => _showMoreOptions(context, music)),
        ]))),
  ...
  Future<void> _showMoreOptions(BuildContext context, MusicProvider music) async {
    final playback = context.read<PlaybackFeaturesProvider>();
    await PlayerActionOverlay.show<void>(context: context, icon: Icons.more_horiz_rounded, title: 'More playback options', child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.speed_rounded), title: const Text('Speed'), trailing: Text('${playback.speed.toStringAsFixed(2)}×'), onTap: () => _showSpeed(context, playback)),
      ListTile(leading: const Icon(Icons.tune_rounded), title: const Text('Pitch'), trailing: Text('${playback.pitch.toStringAsFixed(2)}×'), onTap: () => _showPitch(context, playback)),
...
class _ScrollingSongTitle extends StatelessWidget {
  ...
  const _ScrollingSongTitle({required this.title, required this.style, required this.animation});
  @override
  Widget build(BuildContext context) {
    final direction = Directionality.of(context);
    final painter = TextPainter(text: TextSpan(text: title, style: style), maxLines: 1, textDirection: direction)..layout();
    return LayoutBuilder(builder: (context, constraints) {
      final overflow = painter.width > constraints.maxWidth;
      if (!overflow) return Text(title, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: style);
      final distance = painter.width - constraints.maxWidth + 26;
      return ClipRect(child: SizedBox(height: (style?.fontSize ?? 20) * 1.5, child: AnimatedBuilder(animation: animation, builder: (_, __) => Transform.translate(offset: Offset(-distance * animation.value, 0), child: SizedBox(width: painter.width, child: Text(title, maxLines: 1, style: style))))));
    });
  }
}
...
class _IntelligenceNextCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(color: scheme.primaryContainer.withOpacity(.72), child: Padding(padding: const EdgeInsets.fromLTRB(16, 15, 10, 15), child: Row(children: [
      Icon(Icons.auto_awesome, color: scheme.primary),
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('A thought for your next track', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: scheme.primary)),
        const SizedBox(height: 3),
        Text(item.song.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium),
        Text('${item.confidenceLabel} • ${(item.confidence * 100).round()}% confidence'),
        const SizedBox(height: 3),
        Text(item.reason, maxLines: 2, overflow: TextOverflow.ellipsis),
      ])),
      IconButton(tooltip: 'Play suggestion', icon: const Icon(Icons.play_circle_fill_rounded, size: 34), onPressed: () => music.playSong(item.song, queue: [item.song], startIndex: 0)),
    ])));
  }
}
...
class _IconControl extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => IconButton(tooltip: tooltip, icon: Icon(icon, size: 32), onPressed: onPressed);
}

...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Consumer<MusicProvider>(builder: (_, music, __) => Padding(padding: const EdgeInsets.only(right: 8), child: Center(child: Chip(avatar: Icon(music.activeEngineLabel == 'A' ? Icons.looks_one_rounded : Icons.looks_two_rounded, size: 17), label: Text('Engine ${music.activeEngineLabel}'))))),
        ],
      ),
      body: Consumer<MusicProvider>(builder: (context, music, _) {
        ...
        return ListView(
          ...
          children: [
            ...
            const SizedBox(height: 100, child: AudioVisualizationWidget()),
            _WaveSeekBar(
              value: position,
              max: max,
              onStart: (value) => setState(() => _dragPosition = value),
              onUpdate: (value) => setState(() => _dragPosition = value),
              onEnd: (value) async {
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(AudioFileService.formatDuration(Duration(milliseconds: position.round()))), Text(AudioFileService.formatDuration(duration))])),
            ...
            const SizedBox(height: 8),
            Card(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              IconButton(tooltip: 'Volume', icon: Icon(_volumeIcon(music.volume)), onPressed: () => _showVolume(context, music)),
  ...
  IconData _volumeIcon(double volume) {
    ...
    if (volume < .45) return Icons.volume_down_rounded;
    return Icons.volume_up_rounded;
  }

  Future<void> _showVolume(BuildContext context, MusicProvider music) async {
    await showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheetContext) => SafeArea(child: StatefulBuilder(builder: (_, setSheetState) => Padding(padding: const EdgeInsets.fromLTRB(22, 8, 22, 24), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Row(children: [const Icon(Icons.volume_up_rounded), const SizedBox(width: 12), const Text('Volume', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)), const Spacer(), Text('${(music.volume * 100).round()}%')]),
      Slider(value: music.volume, min: 0, max: 1, divisions: 100, onChanged: (value) { music.setVolume(value); setSheetState(() {}); }),
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [TextButton.icon(onPressed: () { music.setVolume(0); setSheetState(() {}); }, icon: const Icon(Icons.volume_off_rounded), label: const Text('Mute')), TextButton(onPressed: () { music.setVolume(1); setSheetState(() {}); }, child: const Text('100%'))]),
    ]))));
  }
  ...
  Future<void> _showMoreOptions(BuildContext context) async {
    ...
    await showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.equalizer_rounded), title: const Text('Equalizer'), subtitle: Text(eq.isEnabled ? eq.preset : 'Disabled'), onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const EqualizerScreen())); }),
      ListTile(leading: const Icon(Icons.auto_awesome_rounded), title: const Text('Audio effects'), subtitle: Text(effects.effectsEnabled ? 'Bass, width, reverb and loudness' : 'Disabled'), onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const AudioEffectsScreen())); }),
      ...
      ListTile(leading: const Icon(Icons.tune_rounded), title: const Text('Pitch'), trailing: Text('${playback.pitch.toStringAsFixed(2)}×'), onTap: () { Navigator.pop(context); _showPitch(context, playback); }),
      ...
      ListTile(leading: const Icon(Icons.timer_outlined), title: const Text('Sleep timer'), trailing: Text(playback.sleepTimerLabel), onTap: () { Navigator.pop(context); _showSleepTimer(context, playback); }),
    ])));
  }
  ...
  Future<void> _showPitch(BuildContext context, PlaybackFeaturesProvider playback) async {
    await showDialog<void>(context: context, builder: (_) => AlertDialog(title: const Text('Pitch'), content: StatefulBuilder(builder: (context, setState) => Column(mainAxisSize: MainAxisSize.min, children: [Text('${playback.pitch.toStringAsFixed(2)}×'), Slider(min: .5, max: 2, divisions: 30, value: playback.pitch, onChanged: (value) { playback.setPitch(value); setState(() {}); })]))));
  }
...
class _WaveSeekBar extends StatelessWidget {
  ...
  double _valueFor(Offset local, double width) => (local.dx / width).clamp(0.0, 1.0) * max;

  @override
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onHorizontalDragStart: (details) => onStart(_valueFor(details.localPosition, constraints.maxWidth)),
      onHorizontalDragUpdate: (details) => onUpdate(_valueFor(details.localPosition, constraints.maxWidth)),
      onHorizontalDragEnd: (_) => onEnd(value),
      onTapDown: (details) => onStart(_valueFor(details.localPosition, constraints.maxWidth)),
      onTapUp: (details) => onEnd(_valueFor(details.localPosition, constraints.maxWidth)),
      child: SizedBox(height: 64, child: CustomPaint(painter: _WaveSeekPainter(progress: max <= 0 ? 0 : value / max, color: Theme.of(context).colorScheme.primary, muted: Theme.of(context).colorScheme.outlineVariant))),
    );
  });
}
...
class _WaveSeekPainter extends CustomPainter {
  ...
  final Color color;
  final Color muted;
  _WaveSeekPainter({required this.progress, required this.color, required this.muted});
  @override void paint(Canvas canvas, Size size) {
    final count = math.max(36, (size.width / 6).round());
    final step = size.width / count;
    final active = Paint()..color = color..strokeCap = StrokeCap.round..strokeWidth = math.max(2, step * .46);
    final inactive = Paint()..color = muted.withOpacity(.55)..strokeCap = StrokeCap.round..strokeWidth = math.max(2, step * .42);
    for (var i = 0; i < count; i++) {
      final x = step * (i + .5);
      final envelope = .22 + .78 * math.pow(math.sin((i + 2) * .61).abs(), 1.6).toDouble();
      final h = size.height * .12 + envelope * size.height * .34;
      final p = i / count;
      canvas.drawLine(Offset(x, size.height / 2 - h), Offset(x, size.height / 2 + h), p <= progress ? active : inactive);
    }
    final x = size.width * progress.clamp(0.0, 1.0);
    canvas.drawCircle(Offset(x, size.height / 2), 5.5, Paint()..color = color);
  }
  @override bool shouldRepaint(covariant _WaveSeekPainter old) => old.progress != progress || old.color != color || old.muted != muted;
}
...
class _NextCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(color: Theme.of(context).colorScheme.primaryContainer, child: ListTile(leading: const Icon(Icons.auto_awesome), title: const Text('A thought for your next track'), subtitle: Text('${item.song.title}\n$confidence% confidence • ${item.reason}', maxLines: 4, overflow: TextOverflow.ellipsis), trailing: IconButton(icon: const Icon(Icons.play_circle_fill_rounded), onPressed: () => music.playSong(item.song, queue: [item.song], startIndex: 0))));
  }
}

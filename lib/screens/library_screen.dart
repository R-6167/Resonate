...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  @override
  void initState() { super.initState(); _searchController.addListener(_onSearchChanged); }
  void _onSearchChanged() { context.read<LibraryProvider>().searchSongs(_searchController.text); setState(() {}); }

  Future<void> _playSong(BuildContext context, List<SongRef> unused, int index) async {}
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () async {
            final p = context.read<LibraryProvider>(); await p.loadAllSongs();
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Library refreshed')));
          }),
          PopupMenuButton<String>(
            onSelected: (value) { final p = context.read<LibraryProvider>();
              if (['title','artist','album','date_added'].contains(value)) p.setSortBy(value);
              if (['all','favorites','recent'].contains(value)) p.setFilterBy(value);
            },
            itemBuilder: (_) => const [
      ...
      body: Column(children: [
        Padding(padding: const EdgeInsets.all(16), child: TextField(
          ...
          decoration: InputDecoration(hintText: 'Search songs, artists, albums...', prefixIcon: const Icon(Icons.search), suffixIcon: _searchController.text.isNotEmpty ? IconButton(icon: const Icon(Icons.clear), onPressed: _searchController.clear) : null, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
        )),
        Consumer<LibraryProvider>(builder: (context, p, _) => Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
            _StatItem(value: '${p.statistics['totalSongs'] ?? 0}', label: 'Songs'),
            _StatItem(value: '${p.statistics['favoriteCount'] ?? 0}', label: 'Favorites'),
            _StatItem(value: '${p.statistics['totalPlaylists'] ?? 0}', label: 'Playlists'),
          ]),
        )),
        const Divider(),
        Expanded(child: Consumer2<LibraryProvider, MusicProvider>(builder: (context, library, music, _) {
          final songs = library.filteredSongs.isNotEmpty || library.searchQuery.isNotEmpty ? library.filteredSongs : library.allSongs;
          ...
          return ListView.builder(itemCount: songs.length, itemBuilder: (context, index) {
            ...
            return ListTile(
              leading: Icon(Icons.music_note, color: isCurrent ? Theme.of(context).colorScheme.primary : null),
              ...
              selected: isCurrent,
              onTap: () async {
                await music.setQueue(songs, startIndex: index);
...
// Kept private so no public API changes are required by this screen.
class SongRef {}

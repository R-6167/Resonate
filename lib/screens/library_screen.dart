...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _playSong(BuildContext context, List<dynamic> songs, int index) async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Unable to play this song. Check that the music file is still available.')),
      );
    }
  }
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            PopupMenuButton<String>(
              onSelected: (value) {
                final p = context.read<LibraryProvider>();
                if (['title', 'artist', 'album', 'date_added'].contains(value)) {
                  p.setSortBy(value);
                }
                if (['all', 'favorites', 'recent'].contains(value)) {
                  p.setFilterBy(value);
                }
              },
              itemBuilder: (_) => const [
            ...
            Expanded(
              child: Consumer2<LibraryProvider, MusicProvider>(
                builder: (context, library, music, _) {
                  final songs = library.filteredSongs.isNotEmpty ||
                  ...
                  if (songs.isEmpty) {
                    return const Center(
                      child: Text('No songs found. Add a music folder in Library Management.'),
                    );
                  }

...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _addToPlaylist(Song song) async {
    ...
    if (!mounted) return;
    final selected = await showModalBottomSheet<Playlist>(context: context, showDragHandle: true, builder: (sheetContext) => SafeArea(child: ListView(shrinkWrap: true, children: [
      const ListTile(title: Text('Add to playlist'), subtitle: Text('Choose where to save this song.')),
      ListTile(leading: const Icon(Icons.add_rounded), title: const Text('Create new playlist'), onTap: () async {
        final name = TextEditingController(); final description = TextEditingController();
        final result = await showDialog<List<String>>(context: sheetContext, builder: (dialogContext) => AlertDialog(title: const Text('New playlist'), content: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: name, autofocus: true, decoration: const InputDecoration(labelText: 'Name')), const SizedBox(height: 12), TextField(controller: description, decoration: const InputDecoration(labelText: 'Description (optional)'))]), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(dialogContext, [name.text, description.text]), child: const Text('Create'))]));
      ...
      ...provider.playlists.map((playlist) => ListTile(leading: const Icon(Icons.queue_music_rounded), title: Text(playlist.name), subtitle: Text('${playlist.songIds.length} songs'), onTap: () => Navigator.pop(sheetContext, playlist))),
    ])));
  ...
  Future<void> _chooseFolders() async {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: Consumer2<LibraryProvider, MusicProvider>(builder: (context, library, music, _) {
          ...
          if (songs.isEmpty) return const Center(child: Text('No songs found. Choose a music folder or scan your library.'));
          return ListView.builder(itemCount: songs.length, itemBuilder: (_, index) {
            final song = songs[index]; final current = music.currentSong?.id == song.id; final liked = library.isFavoriteSync(song.id);
            return ListTile(selected: current, leading: Icon(current && music.isPlaying ? Icons.equalizer_rounded : Icons.music_note_rounded, color: current ? Theme.of(context).colorScheme.primary : null), title: Text(song.title, maxLines: 1, overflow: TextOverflow.ellipsis), subtitle: Text(song.artist, maxLines: 1, overflow: TextOverflow.ellipsis), trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(tooltip: liked ? 'Unlike' : 'Like', icon: Icon(liked ? Icons.favorite_rounded : Icons.favorite_border_rounded), onPressed: () => library.toggleFavorite(song)), PopupMenuButton<String>(onSelected: (value) async { if (value == 'next') { final added = await music.playNext(song); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(added ? 'Playing next: ${song.title}' : 'Song is already in the queue'))); } else if (value == 'queue') { final added = await music.addToQueue(song); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(added ? 'Added to queue' : 'Song is already in the queue'))); } else if (value == 'playlist') await _addToPlaylist(song); else if (value == 'delete') await library.deleteSong(song.id); }, itemBuilder: (_) => const [PopupMenuItem(value: 'next', child: Text('Play next')), PopupMenuItem(value: 'queue', child: Text('Add to queue')), PopupMenuItem(value: 'playlist', child: Text('Add to playlist')), PopupMenuItem(value: 'delete', child: Text('Remove from library'))])]), onTap: () => _playSong(songs, index));
          });
        })),
      ]),
    );

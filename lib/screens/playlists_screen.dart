...
class PlaylistsScreen extends StatelessWidget {
  ...
  Future<void> _create(BuildContext context) async {
    ...
    await context.read<PlaylistProvider>().createPlaylist(
      result.first,
      description: result.length > 1 ? result[1] : null,
    );
  }
  ...
  Future<void> _refreshSmart(BuildContext context) async {
    ...
    await playlists.refreshSmartPlaylists(
      recommended: intelligence.recommendations.map((r) => r.song).toList(),
      favorites: library.favoriteSongs,
      recent: recent,
    );
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Resonate smart playlists refreshed.')));
    }
  }
        ...
        actions: [
          Consumer<PlaylistProvider>(
            builder: (_, provider, __) => IconButton(
              tooltip: 'Refresh smart playlists',
              onPressed: provider.isSmartRefreshing ? null : () => _refreshSmart(context),
              icon: provider.isSmartRefreshing
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Create playlists or let Resonate prepare smart mixes from your local listening data.', textAlign: TextAlign.center),
                    const SizedBox(height: 18),
                    FilledButton.icon(onPressed: () => _create(context), icon: const Icon(Icons.add), label: const Text('Create playlist')),
                    const SizedBox(height: 8),
                    OutlinedButton.icon(onPressed: () => _refreshSmart(context), icon: const Icon(Icons.auto_awesome), label: const Text('Create smart playlists')),
                  ],
                ),
              ),
            );
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  child: ListTile(
                    leading: CircleAvatar(child: Icon(smart ? Icons.auto_awesome_rounded : Icons.queue_music_rounded)),
...
class _PlaylistDetailScreenState extends State<PlaylistDetailScreen> {
  ...
  Future<void> _refresh() async {
    setState(() => _songs = context.read<PlaylistProvider>().songsFor(widget.playlist.id));
  }

  Future<void> _addSongs() async {
    final library = context.read<LibraryProvider>();
    final songs = await showDialog<List<Song>>(
      context: context,
      builder: (_) => _SongPicker(songs: library.allSongs, initiallySelected: widget.playlist.songIds.toSet()),
    );
    if (songs == null || !mounted) return;
    await context.read<PlaylistProvider>().addSongs(widget.playlist.id, songs);
    await _refresh();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _addSongs, icon: const Icon(Icons.add_rounded), tooltip: 'Add songs'),
        ],
      ),
          ...
          return Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Expanded(child: Text('${songs.length} songs', style: Theme.of(context).textTheme.bodyMedium)),
                    FilledButton.icon(onPressed: () => _playAll(songs), icon: const Icon(Icons.play_arrow_rounded), label: const Text('Play all')),
                  ],
                ),
              ),
              Expanded(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        tooltip: 'Remove',
                        onPressed: () async {
                          await context.read<PlaylistProvider>().removeSong(widget.playlist.id, song.id);
                          await _refresh();
                        },
                      ),
                      onTap: () async {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: Column(
          children: [
            TextField(onChanged: (value) => setState(() => _query = value), decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search')),
            Expanded(
              child: ListView.builder(
      ...
      actions: [
        ...
        FilledButton(
          onPressed: () => Navigator.pop(
            context,
            widget.songs.where((song) => _selected.contains(song.id) && !widget.initiallySelected.contains(song.id)).toList(),
          ),
          child: Text('Add (${_selected.difference(widget.initiallySelected).length})'),
        ),
      ],
    );
  }

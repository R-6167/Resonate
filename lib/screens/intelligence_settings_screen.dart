  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading ? const Center(child: CircularProgressIndicator()) : Consumer<IntelligenceProvider>(builder: (context, intelligence, _) => ListView(padding: const EdgeInsets.only(bottom: 36), children: [
        ...
        _header(context, 'Deep Companion mixes'),
        ListTile(title: const Text('Generated mix length'), subtitle: Text('$_mixMinutes minutes • the target length for new Companion journeys'), trailing: const Icon(Icons.schedule_rounded), onTap: () => _chooseMixLength(context)),
        ...
        FutureBuilder<List<Map<String, dynamic>>>(future: Future.wait([IntelligencePatternStore.readBucket(DateTime.now()), IntelligencePatternStore.readStateProfile()]), builder: (context, snapshot) {
          ...
          return Column(children: [
            ...
            ListTile(leading: const Icon(Icons.restart_alt_rounded), title: const Text('Reset advanced tuning'), subtitle: const Text('Return decision controls and mix controls to conservative defaults; learned memory stays intact.'), onTap: () => _confirmReset(context)),
          ]);
        }),
      ])),
    );

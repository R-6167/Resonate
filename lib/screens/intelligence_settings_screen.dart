...
class _IntelligenceSettingsScreenState extends State<IntelligenceSettingsScreen> {
  ...
  Future<void> _load() async {
    final values = await Future.wait([
      IntelligenceSettingsStore.exploration(), IntelligenceSettingsStore.confidenceThreshold(),
      IntelligenceSettingsStore.automaticQueue(), IntelligenceSettingsStore.artistRepeat(),
      IntelligenceSettingsStore.sessionIntelligence(), IntelligenceSettingsStore.explanations(),
      IntelligenceSettingsStore.learnedEq(), IntelligenceSettingsStore.autopilotCrossfade(),
      IntelligenceSettingsStore.autopilotCrossfadeMs(),
    ]);
    if (!mounted) return;
    setState(() {
      _exploration = values[0] as int; _confidence = values[1] as double; _automaticQueue = values[2] as bool;
      _artistRepeat = values[3] as bool; _sessionIntelligence = values[4] as bool; _explanations = values[5] as bool;
      _learnedEq = values[6] as bool; _crossfade = values[7] as bool; _crossfadeMs = values[8] as int; _loading = false;
    });
  }

  Future<void> _resetTuning() async { await IntelligenceSettingsStore.reset(); await _load(); }
          ...
          : Consumer<IntelligenceProvider>(
              builder: (context, intelligence, _) => ListView(
                ...
                children: [
                  ...
                  ListTile(title: const Text('Autopilot confidence threshold'), subtitle: Text('${(_confidence * 100).round()}% • only stronger predictions can take over automatically')),
                  ...
                  SwitchListTile.adaptive(title: const Text('Use current-session signals'), subtitle: const Text('Let recent skips, completions and artists steer the next decision.'), value: _sessionIntelligence, onChanged: (v) async { setState(() => _sessionIntelligence = v); await IntelligenceSettingsStore.setSessionIntelligence(v); }),
                  SwitchListTile.adaptive(title: const Text('Show recommendation explanations'), subtitle: const Text('Display why a track was selected.'), value: _explanations, onChanged: (v) async { setState(() => _explanations = v); await IntelligenceSettingsStore.setExplanations(v); }),

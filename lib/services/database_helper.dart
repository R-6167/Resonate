...
class DatabaseHelper {
  ...
  Future<int> insertListeningEvent(ListeningEvent event) async {
    try {
      ...
      final duplicate = await db.query(
        ...
        whereArgs: [event.songId, event.startedAt.subtract(const Duration(seconds: 5)).toIso8601String()],
        limit: 1,
      );
  ...
  Future<int> updateListeningEvent(ListeningEvent event) async { try { return await (await database).update(tableListeningEvents, event.toMap(), where: '$columnEventId = ?', whereArgs: [event.id]); } catch (e) { _markDatabaseFailure('database_operation', e); print('Error updating listening event: $e'); return -1; } }
  Future<List<ListeningEvent>> getRecentListeningEvents({int limit = 100}) async { try { final maps = await (await database).query(tableListeningEvents, orderBy: '$columnEventStartedAt DESC', limit: limit); return maps.map(ListeningEvent.fromMap).toList(); } catch (e) { _markDatabaseFailure('database_operation', e); print('Error fetching listening events: $e'); return []; } }
  Future<List<ListeningEvent>> getListeningHistory({int limit = 200, int offset = 0}) async { try { final maps = await (await database).query(tableListeningEvents, orderBy: '$columnEventStartedAt DESC', limit: limit, offset: offset); return maps.map(ListeningEvent.fromMap).toList(); } catch (e) { _markDatabaseFailure('database_operation', e); print('Error fetching listening history: $e'); return []; } }
  Future<Map<String, dynamic>> getListeningHistoryStatistics() async { try { final db = await database; final rows = await db.rawQuery('SELECT COUNT(*) AS events, COALESCE(SUM($columnEventDurationPlayedMs),0) AS played_ms, COALESCE(SUM(CASE WHEN $columnEventCompleted = 1 THEN 1 ELSE 0 END),0) AS completed, COALESCE(SUM(CASE WHEN $columnEventSkipped = 1 THEN 1 ELSE 0 END),0) AS skipped FROM $tableListeningEvents'); final r = rows.first; return {'events': (r['events'] as num?)?.toInt() ?? 0, 'playedMs': (r['played_ms'] as num?)?.toInt() ?? 0, 'completed': (r['completed'] as num?)?.toInt() ?? 0, 'skipped': (r['skipped'] as num?)?.toInt() ?? 0}; } catch (e) { _markDatabaseFailure('database_operation', e); return {'events': 0, 'playedMs': 0, 'completed': 0, 'skipped': 0}; } }
  Future<int> clearListeningHistory() async { try { return await (await database).delete(tableListeningEvents); } catch (e) { _markDatabaseFailure('database_operation', e); print('Error clearing listening history: $e'); return -1; } }
  Future<List<Map<String, dynamic>>> getTransitionCounts(String songId) async { try { return await (await database).rawQuery('SELECT $columnEventSongId AS next_song_id, COUNT(*) AS transition_count FROM $tableListeningEvents WHERE $columnEventPreviousSongId = ? GROUP BY $columnEventSongId ORDER BY transition_count DESC', [songId]); } catch (e) { _markDatabaseFailure('database_operation', e); return []; } }
  Future<int> insertSong(Song song) async { try { return await (await database).insert(tableSongs, {columnSongId:song.id,columnSongTitle:song.title,columnSongArtist:song.artist,columnSongAlbum:song.album,columnSongFilePath:song.filePath,columnSongDuration:song.duration.inMilliseconds,columnSongDateAdded:song.dateAdded.toIso8601String(),columnSongAlbumArt:song.albumArt,columnSongPlayCount:0,columnSongLastPlayed:null}, conflictAlgorithm: ConflictAlgorithm.ignore); } catch (e) { _markDatabaseFailure('database_operation', e); print('Error inserting song: $e'); return -1; } }
  Future<int> insertSongs(List<Song> songs) async { final db=await database; var count=0; for(final song in songs){try{await db.insert(tableSongs,{columnSongId:song.id,columnSongTitle:song.title,columnSongArtist:song.artist,columnSongAlbum:song.album,columnSongFilePath:song.filePath,columnSongDuration:song.duration.inMilliseconds,columnSongDateAdded:song.dateAdded.toIso8601String(),columnSongAlbumArt:song.albumArt,columnSongPlayCount:0,columnSongLastPlayed:null},conflictAlgorithm:ConflictAlgorithm.ignore);count++;}catch(e){_markDatabaseFailure('database_operation',e);print('Error inserting song ${song.title}: $e');}} return count; }
  Future<List<Song>> getAllSongs() async { try { return (await (await database).query(tableSongs,orderBy:'$columnSongTitle ASC')).map(_songFromMap).toList(); } catch(e){_markDatabaseFailure('database_operation',e);print('Error fetching all songs: $e');return [];} }
  Future<List<Song>> searchSongs(String query) async { try { final q='%$query%'; return (await (await database).query(tableSongs,where:'$columnSongTitle LIKE ? OR $columnSongArtist LIKE ? OR $columnSongAlbum LIKE ?',whereArgs:[q,q,q],orderBy:'$columnSongTitle ASC')).map(_songFromMap).toList(); }catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<List<Song>> getSongsByArtist(String artist) async { try{return (await (await database).query(tableSongs,where:'$columnSongArtist = ?',whereArgs:[artist],orderBy:'$columnSongTitle ASC')).map(_songFromMap).toList();}catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<List<Song>> getSongsByAlbum(String album) async { try{return (await (await database).query(tableSongs,where:'$columnSongAlbum = ?',whereArgs:[album],orderBy:'$columnSongTitle ASC')).map(_songFromMap).toList();}catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<int> updateSongPlayCount(String songId) async {
    try {
      final db = await database;
      final rows = await db.query(tableSongs, columns: [columnSongLastPlayed], where: '$columnSongId = ?', whereArgs: [songId], limit: 1);
      if (rows.isEmpty) return 0;
      final last = DateTime.tryParse(rows.first[columnSongLastPlayed]?.toString() ?? '');
      if (last != null && DateTime.now().difference(last).inSeconds < 5) return 0;
      return await db.rawUpdate('UPDATE $tableSongs SET $columnSongPlayCount=COALESCE($columnSongPlayCount,0)+1,$columnSongLastPlayed=? WHERE $columnSongId=?',[DateTime.now().toIso8601String(),songId]);
    } catch(e){_markDatabaseFailure('database_operation',e);return -1;}
  }
  Future<int> deleteSong(String songId) async { try{final db=await database;await db.delete(tableListeningEvents,where:'$columnEventSongId = ? OR $columnEventPreviousSongId = ?',whereArgs:[songId,songId]);await db.delete(tablePlaylistSongs,where:'$columnPlaylistSongSongId = ?',whereArgs:[songId]);await db.delete(tableFavorites,where:'$columnFavoriteSongId = ?',whereArgs:[songId]);return await db.delete(tableSongs,where:'$columnSongId = ?',whereArgs:[songId]);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<int> insertPlaylist(Playlist playlist) async { try{return await (await database).insert(tablePlaylists,{columnPlaylistId:playlist.id,columnPlaylistName:playlist.name,columnPlaylistCreatedAt:playlist.createdAt.toIso8601String(),columnPlaylistDescription:playlist.description},conflictAlgorithm:ConflictAlgorithm.replace);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<List<Playlist>> getAllPlaylists() async { try{return (await (await database).query(tablePlaylists,orderBy:'$columnPlaylistName ASC')).map(_playlistFromMap).toList();}catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<int> addSongToPlaylist(String playlistId,String songId,int position) async { try{return await (await database).insert(tablePlaylistSongs,{columnPlaylistSongPlaylistId:playlistId,columnPlaylistSongSongId:songId,columnPlaylistSongPosition:position},conflictAlgorithm:ConflictAlgorithm.ignore);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<List<Song>> getPlaylistSongs(String playlistId) async { try{return (await (await database).rawQuery('SELECT s.* FROM $tableSongs s INNER JOIN $tablePlaylistSongs ps ON s.$columnSongId=ps.$columnPlaylistSongSongId WHERE ps.$columnPlaylistSongPlaylistId=? ORDER BY ps.$columnPlaylistSongPosition ASC',[playlistId])).map(_songFromMap).toList();}catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<int> removeSongFromPlaylist(String playlistId,String songId) async { try{return await (await database).delete(tablePlaylistSongs,where:'$columnPlaylistSongPlaylistId = ? AND $columnPlaylistSongSongId = ?',whereArgs:[playlistId,songId]);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<int> deletePlaylist(String playlistId) async { try{final db=await database;await db.delete(tablePlaylistSongs,where:'$columnPlaylistSongPlaylistId = ?',whereArgs:[playlistId]);return await db.delete(tablePlaylists,where:'$columnPlaylistId = ?',whereArgs:[playlistId]);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<int> addFavorite(String songId) async { try{return await (await database).insert(tableFavorites,{columnFavoriteSongId:songId,columnFavoriteDateAdded:DateTime.now().toIso8601String()},conflictAlgorithm:ConflictAlgorithm.ignore);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }
  Future<List<Song>> getFavoriteSongs() async { try{return (await (await database).rawQuery('SELECT s.* FROM $tableSongs s INNER JOIN $tableFavorites f ON s.$columnSongId=f.$columnFavoriteSongId ORDER BY s.$columnSongTitle ASC')).map(_songFromMap).toList();}catch(e){_markDatabaseFailure('database_operation',e);return [];} }
  Future<bool> isFavorite(String songId) async { try{return (await (await database).query(tableFavorites,where:'$columnFavoriteSongId = ?',whereArgs:[songId],limit:1)).isNotEmpty;}catch(e){_markDatabaseFailure('database_operation',e);return false;} }
  Future<int> removeFavorite(String songId) async { try{return await (await database).delete(tableFavorites,where:'$columnFavoriteSongId = ?',whereArgs:[songId]);}catch(e){_markDatabaseFailure('database_operation',e);return -1;} }

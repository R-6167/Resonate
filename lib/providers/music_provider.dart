...
class MusicProvider extends ChangeNotifier {
  ...
  MusicProvider({this.audioHandler}) {
    ...
    if (audioHandler is AudioServiceHandler) {
      (audioHandler! as AudioServiceHandler).bindPlaybackController(
        onPlay: () => togglePlayPause(), onPause: pause, onStop: stop, onSeek: seek, onNext: nextSong, onPrevious: previousSong,
      );
    }
  ...
  Future<void> _loadSingle(AudioPlayer player, AndroidEqualizer eq, AndroidLoudnessEnhancer loud, Song song, {bool start = true}) async {
    ...
    await eq.setEnabled(true); await loud.setEnabled(true); await player.setVolume(start ? _volume : 0.0); if (start) await player.play();
  }
  ...
  Future<bool> playSong(Song song, {List<Song>? queue, int startIndex = 0}) async {
    ...
    try {
      ...
      currentSong = song; currentDuration = song.duration; currentPosition = Duration.zero; isPlaying = false; _bindActivePlayerStreams(); notifyListeners();
      ...
      await equalizer.setEnabled(true); await loudnessEnhancer.setEnabled(true); await audioPlayer.setVolume(_volume); await audioPlayer.play();
  ...
  /// Adds recommendations to the active queue without interrupting playback.
  Future<bool> enqueueSongs(List<Song> songs) async {
    ...
    try {
      ...
      if (source is ConcatenatingAudioSource) {
        ...
        await audioPlayer.setAudioSource(ConcatenatingAudioSource(children: rebuilt.map((s) => AudioSource.uri(_audioUri(s.filePath), tag: s)).toList(), useLazyPreparation: true), initialIndex: index, initialPosition: position);
        _queue = rebuilt; _queueIndex = index;
        if (wasPlaying) await audioPlayer.play();
      } else return false;
      notifyListeners(); return true;
    } catch (e) { debugPrint('Queue extension failed: $e'); return false; }
  }
  ...
  Future<bool> performTrueCrossfade({required int milliseconds, String fadeType = 'linear'}) async {
    ...
    try {
      ...
      for (var i = 1; i <= steps; i++) {
        if (!outgoing.playing) break; final linear = i / steps;
    ...
    } catch (e, stack) { debugPrint('True crossfade failed: $e'); debugPrint('$stack'); try { await incoming.stop(); } catch (_) {} try { await outgoing.setVolume(master); } catch (_) {} return false; }

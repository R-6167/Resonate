...
class MusicProvider extends ChangeNotifier {
  ...
  Future<void> _restoreQueue() async {
    ...
    try {
      ...
      notifyListeners();
    } catch (e) { debugPrint('Playback queue restore failed: $e'); }
    finally { _queueRestoreInProgress = false; }
  }
  ...
  Future<void> _persistQueue() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      if (_queue.isEmpty) {
        await prefs.remove(_savedQueueIdsKey); await prefs.remove(_savedQueueIndexKey);
      } else {
  ...
  void _bindActivePlayerStreams() {
    ...
    _playerStateSubscription = player.playerStateStream.listen((state) {
      ...
      if (state.processingState == ProcessingState.completed) {
        currentPosition = currentDuration ?? currentPosition; isPlaying = false; notifyListeners(); _publishServiceState();
    ...
    _positionSubscription = player.positionStream.listen((position) {
      if (currentPosition != position) {
        ...
        if (_activeHistoryEvent != null) { _activeHistoryPositionMs = position.inMilliseconds; _persistResumePosition(); }
        notifyListeners(); _publishServiceState();
      }
  ...
  Future<void> _runAutomaticCrossfade() async { try { await _performTrueCrossfade(milliseconds: _crossfadeDurationMs, fadeType: _crossfadeFadeType); } finally { _automaticCrossfadeInFlight = false; } }
  ...
  Future<void> _advanceAfterCompletion() async {
    ...
    try {
      ...
      if (_repeatMode == PlaybackRepeatMode.one) {
        currentPosition = Duration.zero; await audioPlayer.seek(Duration.zero); await audioPlayer.play(); isPlaying = true; await _startHistoryEvent(currentSong!); _publishServiceState(); notifyListeners(); return;
      }
      if (_queueIndex < _queue.length - 1) { await _playSongInternal(_queue[_queueIndex + 1], queue: _queue, startIndex: _queueIndex + 1); return; }
      if (_repeatMode == PlaybackRepeatMode.all && _queue.isNotEmpty) { await _playSongInternal(_queue.first, queue: _queue, startIndex: 0); return; }
  ...
  Future<bool> playSong(Song song, {List<Song>? queue, int startIndex = 0}) => _serializePlayback(() => _playSongInternal(song, queue: queue, startIndex: startIndex));
  ...
  Future<bool> _playSongInternal(Song song, {List<Song>? queue, int startIndex = 0, bool resume = false}) async {
    ...
    try {
      ...
      await _finishHistoryEvent(); await _stopBoth();
      if (transition != _transitionSerial) return false;
      _activeIsA = true;
      ...
      if (_shuffleEnabled && normalized.length > 1) {
        final selected = normalized[selectedIndex]; final upcoming = <Song>[...normalized]..removeAt(selectedIndex)..shuffle(Random());
        _queue = [selected, ...upcoming]; _queueIndex = 0;
      } else { _queue = normalized; _queueIndex = selectedIndex; }
      currentSong = _queue[_queueIndex]; currentDuration = currentSong!.duration; currentPosition = Duration.zero; isPlaying = false;
      await _persistQueue(); _bindActivePlayerStreams(); notifyListeners();
      ...
      if (resume && _resumeSongId == currentSong!.id && _resumePositionMs > 0) {
        ...
        await audioPlayer.seek(Duration(milliseconds: safeResume)); currentPosition = Duration(milliseconds: safeResume);
      }
      await _enableEffects(audioPlayer, equalizer, loudnessEnhancer); await audioPlayer.setVolume(_volume); await audioPlayer.play();
      isPlaying = true; await _startHistoryEvent(currentSong!); _publishServiceState(); notifyListeners(); return true;
  ...
  Future<void> _finishHistoryEvent({bool completed = false}) async {
    await _queueHistoryOperation(() async {
      ...
      if (wasCompleted) {
        _resumePositionMs = 0; _resumeSongId = null;
      ...
      } else { _persistResumePosition(force: true); }
  ...
  Future<void> _queueHistoryOperation(Future<void> Function() operation) {
    ...
    final next = _historySerial.then((_) async { _historyOperationActive = true; try { await operation(); } finally { _historyOperationActive = false; } });
    _historySerial = next.then<void>((_) {}, onError: (_, __) {}); return next;
  }
  ...
  Future<bool> enqueueSongs(List<Song> songs) async {
    ...
    _queue.addAll(additions); await _persistQueue(); notifyListeners(); return true;
  }
  ...
  Future<bool> playNext(Song song) async {
    ...
    final insertAt = (_queueIndex + 1).clamp(0, _queue.length).toInt(); _queue.insert(insertAt, song); await _persistQueue(); notifyListeners(); return true;
  }
  ...
  void clearUpcomingQueue() {
    ...
    _queue = [..._queue.take(_queueIndex + 1)]; unawaited(_persistQueue()); notifyListeners();
  }
  ...
  Future<void> _loadSingle(AudioPlayer player, AndroidEqualizer eq, AndroidLoudnessEnhancer loud, Song song, {bool start = true}) async {
    await player.setLoopMode(LoopMode.off); await player.setAudioSource(AudioSource.uri(_audioUri(song.filePath), tag: song)); await _enableEffects(player, eq, loud); await player.setVolume(start ? _volume : 0.0); if (start) await player.play();
  }

  Future<bool> performTrueCrossfade({required int milliseconds, String fadeType = 'linear'}) => _performTrueCrossfade(milliseconds: milliseconds, fadeType: fadeType);
  ...
  Future<bool> _performTrueCrossfade({required int milliseconds, String fadeType = 'linear'}) async {
    ...
    _crossfadeInProgress = true; final transition = ++_transitionSerial; final outgoing = audioPlayer; final incoming = inactivePlayer; final incomingEq = inactiveEqualizer; final incomingLoud = inactiveLoudnessEnhancer; final master = _volume;
    try {
      ...
      for (var i = 1; i <= steps; i++) {
        if (transition != _transitionSerial || !outgoing.playing) { try { await incoming.stop(); } catch (_) {} return false; }
      ...
      await _finishHistoryEvent(); await _persistQueue(); _bindActivePlayerStreams(); await _startHistoryEvent(nextSong); _publishServiceState(); notifyListeners(); await outgoing.stop(); return true;
    } catch (e, stack) {
      debugPrint('True crossfade failed: $e'); debugPrint('$stack'); try { await incoming.stop(); } catch (_) {} try { await outgoing.setVolume(master); } catch (_) {}
  ...
  Future<void> togglePlayPause() async {
    ...
    try {
      if (player.playing) {
        ++_transitionSerial; await player.pause(); isPlaying = false; _persistResumePosition(force: true); _publishServiceState(); notifyListeners(); return;
      }
      if (player.audioSource != null) { await player.play(); isPlaying = true; _publishServiceState(); notifyListeners(); }
  ...
  Future<void> pause() async {
    ++_transitionSerial;
    try { await audioPlayer.pause(); isPlaying = false; _persistResumePosition(force: true); _publishServiceState(); notifyListeners(); } catch (_) {}
  }

  Future<void> stop() async {
    ++_transitionSerial;
    try { await _finishHistoryEvent(); await _stopBoth(); isPlaying = false; currentPosition = Duration.zero; _publishServiceState(); notifyListeners(); } catch (_) {}
  }

  Future<void> nextSong() async {
    if (_queue.isEmpty) return;
    final requestedTransition = ++_transitionSerial;
    try {
      if (_queueIndex >= _queue.length - 1) {
        if (_repeatMode == PlaybackRepeatMode.all) await playSong(_queue.first, queue: _queue, startIndex: 0);
        else if (_repeatMode == PlaybackRepeatMode.one && currentSong != null) await playSong(currentSong!, queue: _queue, startIndex: _queueIndex);
        return;
      }
      final nextIndex = _queueIndex + 1;
      if (_crossfadeEnabled && canCrossfadeNext && audioPlayer.playing) {
        _transitionSerial = requestedTransition;
      ...
      if (requestedTransition != _transitionSerial && !audioPlayer.playing) return;
      await playSong(_queue[nextIndex], queue: _queue, startIndex: nextIndex);
    } catch (e) { debugPrint('Next song failed: $e'); }
  }

  Future<void> previousSong() async {
    ++_transitionSerial;
    try {
      if (_queueIndex > 0) { final previousIndex = _queueIndex - 1; await playSong(_queue[previousIndex], queue: _queue, startIndex: previousIndex); }
      else { await audioPlayer.seek(Duration.zero); currentPosition = Duration.zero; if (_activeHistoryEvent != null) _activeHistoryPositionMs = 0; _persistResumePosition(force: true); notifyListeners(); }
    } catch (e) { debugPrint('Previous song failed: $e'); }
  }

  Future<void> seek(Duration position) async {
    final serial = ++_seekSerial;
    try {
      final player = audioPlayer; final duration = player.duration ?? currentDuration ?? Duration.zero;
      ...
      currentPosition = safe; if (_activeHistoryEvent != null) _activeHistoryPositionMs = safe.inMilliseconds;
      _persistResumePosition(force: true); _publishServiceState(); notifyListeners();
    } catch (e) { debugPrint('Seek failed: $e'); }
  }

  Future<void> setVolume(double volume) async {
    _volume = volume.clamp(0.0, 1.0).toDouble();
    try { await audioPlayer.setVolume(_volume); if (!inactivePlayer.playing) await inactivePlayer.setVolume(_volume); notifyListeners(); } catch (_) {}
  }

  Future<void> setQueue(List<Song> songs, {int startIndex = 0}) async {
    if (songs.isEmpty) { await _finishHistoryEvent(); _queue = <Song>[]; _queueIndex = 0; await _persistQueue(); notifyListeners(); return; }
    final index = startIndex.clamp(0, songs.length - 1).toInt(); await playSong(songs[index], queue: songs, startIndex: index);
  }
  ...
  @override
  void dispose() {
    _playerStateSubscription?.cancel(); _positionSubscription?.cancel(); _durationSubscription?.cancel(); _volumeSubscription?.cancel(); _interruptionSubscription?.cancel(); _noisySubscription?.cancel();
    _playerA.dispose(); _playerB.dispose(); super.dispose();
  }
}
